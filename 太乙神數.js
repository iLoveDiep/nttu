...
House.prototype =
{
	...
	checkScheming : function ()
	{
		...
		for (var i = 0; i < 12; ++i)
		{
			if (_earthlyBranches[i] == parseInt(localStorage.getItem("_hour_earthlyBranch")))
			{
				_scheming = _earthlyBranches[i];
			}
		}
	...
	{
		for(var i = 0; i < this.starsAndGods.length; ++i)
		{
			if (i % 3 == 0)
			{
				this.contentOfStarsAndGods1 += this.starsAndGods[i] + " ";
			}
			else if (i % 3 == 1)
			{
				this.contentOfStarsAndGods2 += this.starsAndGods[i] + " ";
			}
			else
			{
				this.contentOfStarsAndGods3 += this.starsAndGods[i] + " ";
			}
		}
...
function checkMainCalculationAndCustomCalculationAndDeterminded()
{
	...
	{
		for(var i = __locationOfWenChang; i < __locationOfTaiYi + 8; ++i)
		{
			mainCalculation += _orderOfNumberOfVena[i];
		}
	}
	else
	{
		for(var i = __locationOfWenChang; i < __locationOfTaiYi; ++i)
		{
			mainCalculation += _orderOfNumberOfVena[i];
		}
	}
	...
	{
		for(var i = __locationOfFirstStrike; i < __locationOfTaiYi + 8; ++i)
		{
			customerCalculation += _orderOfNumberOfVena[i];
		}
	}
	else
	{
		for(var i = __locationOfFirstStrike; i < __locationOfTaiYi; ++i)
		{
			customerCalculation += _orderOfNumberOfVena[i];
		}
	}
	...
	{
		for(var i = __locationOfFixedSight; i < __locationOfTaiYi + 8; ++i)
		{
			determinded += _orderOfNumberOfVena[i];
		}
	}
	else
	{
		for(var i = __locationOfFixedSight; i < __locationOfTaiYi; ++i)
		{
			determinded += _orderOfNumberOfVena[i];
		}
	}
...
function translate(_starsAndGod_)
{
	...
	for (var i = 0; i < 16; ++i)
	{
		if (_starsAndGod_ == orderOfSixteenthHouse[i])
		{
			_index = i;
		}
	}
...
for(var i = 0; i < 16; ++i)
{
	...
	map[i].checkTaiYi();
	map[i].checkWenChang();
	map[i].checkFirstStrike();
	map[i].checkJunJi();
	map[i].checkChenJi();
	map[i].checkMinJi();
	map[i].checkFiveBlessings();
	map[i].checkGreatTour();
	map[i].checkSmallTour();
	map[i].checkFourGods();
	map[i].checkTianYi();
	map[i].checkB();
	map[i].checkStraightCharacter();
	map[i].checkFlyingCharacter();
	map[i].checkYangJiu();
	map[i].checkOneHundredSix();
}
for(var i = 0; i < 16; ++i)
{
	map[i].checkFixedSight();
	map[i].checkMainGeneral();
	map[i].checkChiefOfficer();
	map[i].checkGeneralKe();
	map[i].checkGuestParticipant();
	map[i].checkDingGeneral();
	map[i].checkDifiniteGeneral();
}
